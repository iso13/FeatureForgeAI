// src/utils/enforceDeclarative.backup.ts
/**
 * FeatureForge AI
...
function enforceDeclarativeSteps(content: string): string {
  let transformed = content
    ...
    .replace(
      /"User has been added successfully"/gi,
    ...
    .replace(
      /When I fill in the user details with valid data/gi,
      "When I enter valid user information",
    )
    .replace(
      /When I fill in the user details with invalid data/gi,
      "When I enter invalid user information",
    )
    .replace(
      /When I fill in the user details with an existing username/gi,
      "When I enter user information with existing username",
    )
    .replace(
      /When I fill in the user details(?!\s+with)/gi,
      "When I enter user information",
    )
    .replace(
      /When I click on the "([^"]*)" button without filling in the details/gi,
      'When I submit the "$1" action without entering details',
    )

    // Transform "I should see" to more generic patterns
    .replace(
      /Then I should see a success message/gi,
      "Then a success message should be displayed",
    )
    .replace(
      /Then I should see an error message/gi,
      "Then an error message should be displayed",
    )
    .replace(
      /Then I should see a duplicate username error message/gi,
      "Then a duplicate username error should be displayed",
    )
    .replace(
      /Then I should see an error message indicating that all fields are required/gi,
      "Then a required fields error should be displayed",
    )
    .replace(
      /Then I should be redirected back to the users list page/gi,
      "Then I should return to the users list",
    )

    // Transform outcome statements to be more generic
    .replace(
      /And the new user should be added to the system/gi,
      "And the user should be created successfully",
    )
    .replace(
      /And the new user should not be added to the system/gi,
  ...
  transformed = transformed.replace(
    /(\n\s*)When the user submits/gm,
    "$1And the user submits",
  );
  ...
  while ((match = scenarioRegex.exec(transformed)) !== null) {
    ...
    if (whenCount > 1) {
      console.warn(
        `🔧 Fixed multiple When statements in: ${block.split("\n")[0]}`,
      );
    }
    if (whenCount > 1 && thenCount > 1) {
      console.warn(
        `⚠️ Detected multiple When→Then pairs in:\n  → ${block.split("\n")[0]}`,
      );
    }
  }
...
function removeRedundantGivensFromScenarios(content: string): string {
  ...
  const backgroundMatch: RegExpMatchArray | null = content.match(
    /Background:\s*([\s\S]*?)(?=\n\s*Scenario:)/,
  );
  ...
  const backgroundGivens: string[] = backgroundMatch[1]
    ...
    .filter(
      (line: string) => line.startsWith("Given ") || line.startsWith("And "),
    )
  ...
  return content.replace(
    scenarioRegex,
    (match: string, scenarioHeader: string, scenarioBody: string) => {
      const lines: string[] = scenarioBody
        .trim()
        .split("\n")
        .map((line: string) => line.trim());
      ...
      for (const line of lines) {
        ...
        if (
          ...
          (line.startsWith("And ") && filteredLines.length === 0)
        ) {
          const normalizedLine: string = line
            .replace(/^And /, "Given ")
            .toLowerCase();
          const isDuplicate: boolean = backgroundGivens.some(
            (bgGiven: string) => {
          ...
          if (!isDuplicate) {
            ...
            console.warn(
              `🔧 Removing redundant Given: "${line}" (already in Background)`,
            );
          }
        } else {
          filteredLines.push(line);
  ...
  return content.replace(
    ...
    (match: string, scenarioHeader: string, scenarioBody: string) => {
      ...
      if (scenarios.length > 1) {
        ...
        return scenarios
          .map((scenarioLines: string[], index: number) => {
            const originalTitle: string = scenarioHeader
              .replace("Scenario: ", "")
              .replace("\n", "");
            const newTitle: string = generateScenarioTitle(
              scenarioLines,
              originalTitle,
              index,
            );
...
function generateScenarioTitle(
  ...
  const whenLine: string | undefined = scenarioLines.find((line: string) =>
  ...
  if (
    whenLine.includes("request user details") ||
    whenLine.includes("search")
  ) {
...
function validateGherkinQuality(content: string): string[] {
  ...
  if (/should see a(?!\s+")|should see an(?!\s+")/gi.test(content)) {
    warnings.push(
      "❌ Vague assertions found - specify exactly what should be seen",
    );
  }
  ...
  if (/click|button|field(?!\s+in)|page(?!\s+is)/gi.test(content)) {
    warnings.push(
      "⚠️ UI-coupled language detected - consider more declarative approach",
    );
  }
  ...
  const scenarioMatches = content.match(
    /Scenario:[\s\S]*?(?=\nScenario:|\n*$)/g,
  );
  ...
  scenarios.forEach((scenario: string, index: number) => {
    ...
    if (whenCount > 1) {
      warnings.push(
        `🔄 Scenario ${index + 1} has ${whenCount} When statements - should be split`,
      );
    }

    // Check scenario length
    const stepLines: string[] = scenario
      .split("\n")
      .filter((line: string) => /^\s*(Given|When|Then|And|But)\s/.test(line));
    if (stepLines.length > 6) {
      warnings.push(
        `📏 Scenario ${index + 1} too long (${stepLines.length} steps) - consider splitting`,
      );
    }
  });
  ...
  if (
    backgroundMatch &&
    scenarioGivensMatch &&
    scenarioGivensMatch.length > 0
  ) {
    warnings.push(
      "🔄 Found Given statements in scenarios despite Background - may be redundant",
    );
  }
