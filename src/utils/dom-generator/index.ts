...
export class DOMGenerator {
    ...
    try {
      ...
      try {
        await this.page.waitForSelector(
          'div[id="root"], div[id="app"], main, [data-testid], button, input, textarea, select, .playground',
          { timeout: 10000 },
        );
      } catch (e) {
        console.log(
          "⚠️  Standard selectors not found, continuing with full page analysis...",
        );
      }
      ...
      if (
        bodyText?.includes("You need JavaScript") ||
        (bodyText?.trim().length || 0) < 50
      ) {
        console.log(
          "⚠️  Page appears to need JavaScript or has minimal content",
        );
      ...
      if (elements.length === 0) {
        console.log(
          "⚠️  No interactive elements found. Analyzing page structure...",
        );
  ...
  generateFeature(
    ...
    return generator.generateFeature(analysis, featureName, scenarioCount); // ← Add scenarioCount parameter
  }
