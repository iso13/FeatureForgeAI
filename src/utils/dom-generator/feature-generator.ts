...
import type { AnalysisResult, InteractiveElement, ScenarioTemplate } from './types.js';
...
export class FeatureGenerator {
  generateFeature(analysis: AnalysisResult, featureName: string): string {
    ...
    if (inputs.length >= 2 && buttons.length >= 1) {
      scenarios = this.generateLoginScenarios(inputs, buttons);
    }
  ...
  private generateLoginScenarios(inputs: InteractiveElement[], buttons: InteractiveElement[]): string[] {
    ...
    const allScenarios = this.getAllLoginScenarioTemplates(usernameField, passwordField, loginButton);
    
    // Always include successful login scenario (most important)
    const scenarios = [allScenarios[0].content];
    console.log(`   ✅ Added: ${allScenarios[0].name}`);

    // Randomly select remaining scenarios
  ...
  private findElementByKeywords(elements: InteractiveElement[], keywords: string[]): InteractiveElement {
  ...
  private buildFeatureFile(tag: string, featureName: string, pageTitle: string, scenarios: string[]): string {
    return `${tag}
Feature: ${featureName}
...
${scenarios.join('\n\n')}`;
  ...
  generateScenariosByType(elements: InteractiveElement[], type: 'login' | 'form' | 'navigation' | 'shopping'): ScenarioTemplate[] {
    switch (type) {
      case 'login':
        const buttons = elements.filter(el => el.type === 'button' || el.action === 'click');
        const inputs = elements.filter(el => el.type === 'input' || el.action === 'enter text in');
        if (inputs.length >= 2 && buttons.length >= 1) {
          return this.getAllLoginScenarioTemplates(inputs[0], inputs[1], buttons[0]);
        }
        break;
  ...
  private generateFormScenarios(elements: InteractiveElement[]): ScenarioTemplate[] {
    return [
      {
  ...
  private generateNavigationScenarios(elements: InteractiveElement[]): ScenarioTemplate[] {
    return [
      {
  ...
  private generateShoppingScenarios(elements: InteractiveElement[]): ScenarioTemplate[] {
    return [
      {
