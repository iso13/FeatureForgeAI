...
export class PageAnalyzer {
  ...
  async debugPageStructure(): Promise<void> {
    try {
      ...
      if (dynamicIndicators.length > 0) {
        console.log(
          `   🔄 Dynamic content indicators: ${dynamicIndicators.join(", ")}`,
        );
      }
  ...
  private async detectFrameworks(): Promise<string[]> {
    return await this.page.evaluate(() => {
      ...
      if (document.querySelector("[data-reactroot]"))
        indicators.push("React (DOM)");
      if (document.querySelector("[data-reactroot], #root, #app"))
        indicators.push("React App");
      if (document.querySelector("[ng-app], [ng-version]"))
        indicators.push("Angular (DOM)");
  ...
  private async checkDynamicContent(): Promise<string[]> {
    return await this.page.evaluate(() => {
      ...
      if (
        document.querySelector('[class*="skeleton"], [class*="placeholder"]')
      ) {
  ...
  private providePerformanceRecommendations(metrics: {
    ...
    if (metrics.domInteractive > 3000) {
      console.log(
        "   ⚠️  DOM interactive is slow (>3s) - consider reducing DOM complexity",
      );
    }

    if (metrics.loadComplete > 5000) {
      console.log(
        "   ⚠️  Page load is slow (>5s) - consider optimizing resources",
      );
    }

    if (metrics.firstContentfulPaint > 2500) {
      console.log(
        "   ⚠️  First paint is slow (>2.5s) - consider optimizing critical resources",
      );
    }

    if (metrics.responseTime > 1000) {
      console.log(
        "   ⚠️  Server response is slow (>1s) - consider server optimization",
      );
    }
    ...
    try {
      ...
      const a11yIssues = await this.page.evaluate(() => {
        ...
        if (emptyButtons.length > 0) {
          issues.push(
            `${emptyButtons.length} buttons without accessible names`,
          );
        }
