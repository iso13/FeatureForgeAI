...
export function enforceDeclarativeSteps(content: string): string {
  ...
  if (violations.length > 0) {
    ...
    violations.forEach((violation, index) => {
      console.log(`  ${index + 1}. ${violation.type.toUpperCase()}: ${violation.line}`);
 ...
 */
function applyDeclarativeTransformations(content: string, violations: BddViolation[]): string {
  const declarativeRules: DeclarativeRule[] = [
    ...
    {
      pattern: /Then (?:the )?['"]([^'"]+)['"] and ['"]([^'"]+)['"] buttons should be visible/gi,
    ...
    {
      pattern: /When I enter "([^"]+)" in (?:the )?"([^"]+)" (?:field|input|box)/gi,
      replacement: 'When the user specifies "$1" for $2',
      description: 'Convert input entry to data specification'
    },
    {
      pattern: /When I select "([^"]+)" from (?:the )?"([^"]+)" (?:dropdown|select|menu)/gi,
    ...
    {
      pattern: /Then I should see (?:a |an )?"([^"]+)" (?:message|notification)/gi,
    ...
    {
      pattern: /When I (?:click |press )?(?:the )?"?(?:submit|save|create|update)"? button/gi,
    ...
    {
      pattern: /Then (?:I should see )?(?:a )?success(?:ful)? (?:message|confirmation)/gi,
    ...
    {
      pattern: /When I click (?:on )?(?:the )?['"]Discard Recording['"] button/gi,
    ...
    {
      pattern: /Then the audio recording should be saved to the patient's profile/gi,
 ...
 */
function fixRedundantGivenSteps(content: string, violations: BddViolation[]): string {
  ...
  transformed = transformed.replace(scenarioRegex, (match, scenarioLine, scenarioBody) => {
    const scenarioSteps = extractStepsFromText(scenarioBody);
    
    // Check for Given steps that duplicate Background
    const redundantGivens = scenarioSteps.filter(step => 
      step.type === 'Given' && 
      backgroundSteps.some(bgStep => 
        bgStep.text.toLowerCase() === step.text.toLowerCase()
      )
    );
    ...
    redundantGivens.forEach(redundant => {
      const redundantPattern = new RegExp(`^\\s*Given\\s+${escapeRegex(redundant.text)}\\s*$`, 'gm');
 ...
 */
function enforceProperBackground(content: string, violations: BddViolation[]): string {
  ...
  if (!content.includes('Background:')) {
    ...
    if (commonGivens.length > 0) {
      violations.push({
        ...
        suggestion: 'Consider moving common Given steps to a Background section'
      });
    }
  }
 ...
 */
function validateScenarioStructure(content: string, violations: BddViolation[]): void {
  ...
  while ((match = scenarioRegex.exec(content)) !== null) {
    ...
    if (whenSteps > 1 && thenSteps > 1) {
      violations.push({
        ...
        line: scenarioName,
        suggestion: 'Consider splitting into multiple scenarios (one When→Then per scenario)',
        scenarioName
      });
    }
    ...
    steps.forEach(step => {
      const uiKeywords = ['click', 'button', 'field', 'input', 'dropdown', 'menu', 'link', 'page'];
      ...
      if (hasUiKeywords) {
        violations.push({
          ...
          suggestion: 'Consider focusing on business intent rather than UI interactions',
          scenarioName
        });
      }
    });
 ...
 */
function extractStepsFromText(text: string): Array<{type: string, text: string}> {
  ...
  while ((match = stepPattern.exec(text)) !== null) {
    ...
    const stepType = (stepKeyword === 'And' || stepKeyword === 'But') ? lastStepType : stepKeyword;
...
export function generateBddQualityReport(content: string): {
  ...
  if (violations.some(v => v.type === 'imperative')) {
    recommendations.push('Transform imperative steps to focus on business outcomes');
  }
  ...
  if (violations.some(v => v.type === 'ui_focused')) {
    recommendations.push('Focus on business intent rather than UI interactions');
  }
  
  if (violations.some(v => v.type === 'multiple_when_then')) {
    recommendations.push('Split complex scenarios into focused single-purpose scenarios');
  }
  
  // Calculate quality score (0-100)
  const maxViolations = 20; // Arbitrary baseline
  const score = Math.max(0, Math.round((1 - violations.length / maxViolations) * 100));
...
export function validateBddPractices(content: string): boolean {
  ...
  const hasRedundantGivens = !hasBackground && findCommonGivenSteps(content).length > 0;
  const hasImperativeSteps = /When I (?:click|fill|enter|select|go to|navigate)/i.test(content);
