...
export function compareWithSnapshot(
  ...
  const snapshot = fs
    .readFileSync(path.resolve(snapshotFilePath), "utf-8")
    .trim();
...
export function validateTopKDocuments(
  docs: any[],
  expectedCount: number,
): void {
  if (!docs || docs.length !== expectedCount) {
    throw new Error(
      `Expected ${expectedCount} documents, but got ${docs?.length ?? 0}`,
    );
  }
}

export function checkDocsContainKeywords(
  docs: any[],
  keywords: string[],
): void {
...
export function assertSummaryMentions(
  summary: string,
...
export function assertFallbackMessage(
  summary: string,
