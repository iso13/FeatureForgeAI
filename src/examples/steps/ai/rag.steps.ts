...
const clauseSet: Clause[] = [
  {
    ...
    content: "This contract includes a confidentiality clause.",
  },
  {
    id: "C002",
    type: "payment",
...
Given(
  "the RAG system has indexed contract documents containing clauses for confidentiality and payment terms",
...
When(
  "I request a summary of a contract missing indemnity clauses",
  function (this: CustomWorld) {
    if (!this.rag) throw new Error("RAGEngine not initialized");
...
When("I re-request the summary", function (this: CustomWorld) {
  if (!this.rag) throw new Error("RAGEngine not initialized");
  this.summary = this.rag.requestSummary([
    "confidentiality",
    "payment",
    "indemnity",
  ]);
...
Then(
  "the system should retrieve relevant context for each clause",
  function (this: CustomWorld) {
    if (!this.rag) throw new Error("RAGEngine not initialized");
...
Then(
  "the system should generate a summary including confidentiality and payment terms",
  function (this: CustomWorld) {
...
Then(
  "the summary should match the retrieved content",
  function (this: CustomWorld) {
...
Then(
  "the system should log a warning about the missing context",
  function (this: CustomWorld) {
    if (!this.rag) throw new Error("RAGEngine not initialized");
    const log = this.rag
      .getLogs()
      .find((l: string) => l.toLowerCase().includes("warning"));
    expect(log).to.exist;
  },
);

Then(
  "the generated summary should note the missing indemnity information",
  function (this: CustomWorld) {
    const summary = this.summary ?? "";
...
Then(
  "each included clause should be mapped to a source document ID",
  function (this: CustomWorld) {
    if (!this.rag) throw new Error("RAGEngine not initialized");
    const clauseIds = this.rag.getRetrievedClauseIds();
...
Then(
  "the summary should comply with regulatory standards for transparency",
  function (this: CustomWorld) {
    const summary = this.summary ?? "";
...
Given(
  "a new indemnity clause is added to the contract",
  function (this: CustomWorld) {
    if (!this.rag) throw new Error("RAGEngine not initialized");
    this.rag.addClause({
      id: "C003",
      type: "indemnity",
      content: "Indemnity clause added for liability coverage.",
    });
  },
);

Then(
  "the new clause should be included in the updated AI summary",
  function (this: CustomWorld) {
