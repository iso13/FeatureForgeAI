// src/steps/ai/weaviate.steps.ts
/**
 * FeatureForge AI
...
Given(
  "helpful documents are available to search",
  { timeout: 30000 },
  async function (this: CustomWorld) {
    await withSpan(
      "loadDocuments",
...
Then(
  "the AI should give me a short summary based on those documents",
  async function (this: CustomWorld) {
...
Then(
  "the summary should say something like {string}",
...
Then(
  "the AI summary should be no longer than {int} sentences",
  async function (this: CustomWorld, maxSentences: number) {
    if (!this.summary) throw new Error("Summary is undefined");
...
Then(
  "the AI summary should mention a keyword like {string} or {string}",
  async function (this: CustomWorld, keyword1: string, keyword2: string) {
    if (!this.summary) throw new Error("Summary is undefined");

    const summary = this.summary.toLowerCase();
    const containsAny = [keyword1, keyword2].some(
      (k) =>
        summary.includes(k.toLowerCase()) ||
        summary.includes(k.split(" ")[0].toLowerCase()),
    );
...
Then(
  "I should see the top {int} most relevant documents",
...
Then(
  "those documents should talk about {string}",
...
Then(
  "it should ask me to try a different question",
  async function (this: CustomWorld) {
    if (!this.summary) throw new Error("Summary is undefined");
