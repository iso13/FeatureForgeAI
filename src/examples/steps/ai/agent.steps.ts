...
Given(
  "the agent previously failed due to missing data",
  function (this: CustomWorld) {
...
When(
  "the AI agent is instructed to notify investors of a capital call",
  async function (this: CustomWorld) {
    // Do NOT re-instantiate agent here – it’s already created in Background
    this.workflowSteps =
      await this.agent!.runInvestorNotificationWorkflow("fund-001");
  },
);

When(
  "the AI agent attempts to generate a capital call summary",
  async function (this: CustomWorld) {
    this.workflowSteps =
      await this.agent?.runPartialWorkflow("generate-summary");
  },
);

When(
  "the AI agent attempts to send notifications",
  async function (this: CustomWorld) {
    this.workflowSteps =
      await this.agent?.runPartialWorkflow("send-notifications");
  },
);

When(
  "the administrator provides the missing investor records",
  function (this: CustomWorld) {
...
Then(
  "the agent should generate a capital call summary",
  function (this: CustomWorld) {
    const step = this.agent
      ?.getSteps()
      .find((s) => s.tool === "LLM" && s.output.includes("Summary generated"));
    expect(step).to.exist;
  },
);

Then(
  "the agent should validate compliance of the generated summary",
  function (this: CustomWorld) {
    const step = this.agent
      ?.getSteps()
      .find((s) => s.tool === "ComplianceChecker");
    expect(step?.output).to.include("Compliant");
  },
);

Then(
  "the agent should send the notification to each investor",
  function (this: CustomWorld) {
    const step = this.agent
      ?.getSteps()
      .find(
        (s) => s.tool === "EmailService" && s.output.includes("Email sent"),
      );
    expect(step).to.exist;
  },
);
...
Then(
  "the agent should log an error indicating missing data",
  function (this: CustomWorld) {
    const errorStep = this.agent
      ?.getLogs()
      .find(
        (log) =>
          log.type === "error" && log.message.toLowerCase().includes("missing"),
      );
...
Then(
  "the agent should halt the workflow and notify the administrator",
  function (this: CustomWorld) {
    const halted = this.agent?.getStatus();
    expect(halted).to.equal("halted");
    const notification = this.agent
      ?.getLogs()
      .find((log) => log.message.includes("Administrator notified"));
    expect(notification).to.exist;
  },
);

Then(
  "the agent should retry delivery with exponential backoff",
  function (this: CustomWorld) {
    const retrySteps =
      this.agent
        ?.getSteps()
        .filter(
          (s) => s.tool === "EmailService" && s.output.includes("retry"),
        ) || [];
...
Then("the agent should log each retry attempt", function (this: CustomWorld) {
  const retryLogs =
    this.agent
      ?.getLogs()
      .filter((log) => log.message.includes("Retrying email delivery")) || [];
  expect(retryLogs.length).to.be.greaterThan(0);
});

Then(
  "the agent should complete all remaining steps successfully",
  function (this: CustomWorld) {
