    ...
    await withSpan(
      ...
      async () => {
        ...
        this.fundAgreements =
          JSON.parse(
            fs.readFileSync(
              path.join(basePath, "fund_agreements.json"),
              "utf-8",
            ),
          ) || [];
        this.capitalCalls =
          JSON.parse(
            fs.readFileSync(path.join(basePath, "capital_calls.json"), "utf-8"),
          ) || [];
...
When(
  "I tell the AI to send a capital call notification",
  async function (this: CustomWorld) {
    await withSpan(
      "generateSummary",
      async () => {
...
Then(
  "it should write a summary of the capital call",
  async function (this: CustomWorld) {
    await withSpan(
      "validateSummaryExists",
      async () => {
...
Then(
  "make sure the summary follows all compliance rules",
  async function (this: CustomWorld) {
    await withSpan(
      "validateCompliance",
      async () => {
...
Then(
  "record each step it takes with a timestamp",
  async function (this: CustomWorld) {
    await withSpan(
      "verifyNotificationTimestamps",
      async () => {
        const allHaveTimestamps = (this.notificationLogs || []).every(
...
Given(
  "some investor records are incomplete",
  async function (this: CustomWorld) {
    await withSpan(
      "injectIncompleteInvestorRecord",
      async () => {
...
When("the AI tries to create the summary", async function (this: CustomWorld) {
  await withSpan(
    "attemptGenerateSummaryWithMissingData",
    async () => {
      try {
        const call = this.capitalCalls?.[0];
        const agreement = this.fundAgreements?.[0];
...
Then(
  "it should log an error about the missing info",
  async function (this: CustomWorld) {
    await withSpan(
      "checkMissingInvestorInfo",
      async () => {
...
Then("stop and alert the administrator", async function (this: CustomWorld) {
  await withSpan(
    "checkStopWorkflow",
    async () => {
      expect(this.error || (this.investors || []).some((inv) => !inv.name)).to
        .be.ok;
    },
    {},
    this.stepSpan,
...
When(
  "the AI tries to send out the messages",
  async function (this: CustomWorld) {
    await withSpan(
...
Then(
  "it should keep retrying with longer delays",
  async function (this: CustomWorld) {
    await withSpan(
      "verifyRetryLogic",
      async () => {
...
Then("log each attempt to send", async function (this: CustomWorld) {
  await withSpan(
    "verifyRetryLogs",
    async () => {
      const allLogged = (this.notificationLogs || []).every(
        (log) => log.retry && log.timestamp,
      );
...
Given(
  "the AI failed earlier because investor data was missing",
  async function (this: CustomWorld) {
    await withSpan(
      "simulatePreviousDataFailure",
      async () => {
...
Then(
  "it should pick up where it left off and finish everything",
  async function (this: CustomWorld) {
    await withSpan(
      "validateWorkflowCompletion",
      async () => {
        expect((this.notificationLogs || []).length).to.equal(
