...
import { StockPricePredictor } from "../../examples/assets/predictor/stockPricePredictor";
...
Given(
  "I have trained the stock price predictor with historical data",
  async function () {
...
When(
  "I predict the stock price for a future date {string}",
...
When(
  "I predict the stock price for a historical date {string}",
  function (date: string) {
    const inputData = predictor.getInputForDate(date);
    predictedPrice = predictor.predict(inputData);
    console.log(`Predicted stock price for ${date}: ${predictedPrice}`);
