...
Then('the AI should give me a short summary based on those documents', async function () {
  if (!this.summary || typeof this.summary !== 'string' || !this.summary.trim()) {
    throw new Error('No summary was generated by the AI.');
  }
  this.lastSummaryOutput = this.summary;
  console.log('📄 AI Summary:', this.summary);
});

Then('the summary should say something like {string}', async function (expectedPhrase: string) {
  const summary = this.lastSummaryOutput;

  if (!summary || typeof summary !== 'string') {
    throw new Error('No summary was generated. Cannot validate.');
  }

  assertSummaryMentions(summary, [
    expectedPhrase,
